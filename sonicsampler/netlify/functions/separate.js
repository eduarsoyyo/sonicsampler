// Stem separation using Demucs htdemucs_6s — 6 stems, max quality settings.
// htdemucs_6s separates: drums, bass, guitar, piano, vocals, other
// Quality parameters: shifts=5, overlap=0.5 → Moises/Lalal-level quality

const https = require("https");

const TOKEN     = process.env.REPLICATE_API_TOKEN;

// htdemucs model on Replicate — supports htdemucs_6s via model param
...
exports.handler = async (event) => {
  ...
  try {
    // ── Start prediction with htdemucs_6s + max quality ──────────────────
    const start = await rep("POST", "/v1/predictions", {
      ...
      input: {
        ...
        output_format: "mp3",
        clip_mode:     "rescale",
        shifts:        5,               // ★ 5 random shifts → much better quality (vs 1)
        overlap:       0.5,             // ★ 50% overlap → better boundaries (vs 0.25)
        mp3_bitrate:   320,             // ★ 320kbps output
      },
    });
    ...
    // ── Poll up to 90s (shifts=5 takes longer than shifts=1) ─────────────
    for (let i = 0; i < 30; i++) {
      await sleep(3000);
      ...
      if (status === "succeeded") {
        // output = { drums, bass, guitar, piano, vocals, other } with mp3 URLs
        return {
          statusCode: 200,
          headers: { "Content-Type": "application/json" },
    ...
    // Still running — return predId for client-side polling
    return {
      statusCode: 202,
      headers: { "Content-Type": "application/json" },
